...
flags.DEFINE_string('front_ip_address', '192.168.0.1', 'ip address of the device capturing')
flags.DEFINE_string('back_ip_address', '192.168.0.1', 'ip address of the device capturing')
flags.DEFINE_string('plate_weights', './checkpoints/yolov4-416',
...
flags.DEFINE_string('port', '544', ' port to be used to access the cameras')
...
def detect_plate(_argv):
    ...
    if FLAGS.framework == 'tflite':
        interpreter = tf.lite.Interpreter(model_path=FLAGS.plate_weights)
    else:
        saved_model_loaded = tf.saved_model.load(FLAGS.plate_weights, tags=[tag_constants.SERVING])
    ...
    f_image, b_image = get_frames(FLAGS.front_ip_address, FLAGS.back_ip_address, FLAGS.front_password, FLAGS.back_password, FLAGS.front_user_name,
                        FLAGS.back_user_name, FLAGS.port)
    # loop through images in list and run Yolov4 model on each
    for image_path in (f_image, b_image):
