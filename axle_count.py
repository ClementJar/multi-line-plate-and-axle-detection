...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            cv2.line(frame, (int(3 * original_w / 6), 0), (int(3 * original_w / 6), original_h), (0, 255, 0), thickness=2)
            # get center_y
            center_y = int(((bbox[0]) + (bbox[2])) / 2)

            if center_y <= int(3 * original_w / 6 + original_w / 30) and center_y >= int(
                    3 * original_w / 6 - original_w / 30):
